...
public class CollectorSamplerTest {
  ...
  public void mostNegativeNumberDefence() {
    ...
    assertThat(sampler.isSampled(TestObjects.span(Long.MIN_VALUE)))
        .isEqualTo(sampler.isSampled(TestObjects.span(Long.MAX_VALUE)));
  }
  ...
  public void debugWins() {
    ...
    assertThat(sampler.isSampled(TestObjects.span(Long.MIN_VALUE).toBuilder().debug(true).build()))
        .isTrue();
  }
  ...
  public void retain10Percent() {
    ...
    long passCount = Stream.of(TestObjects.LOTS_OF_SPANS).parallel().filter(sampler::isSampled).count();

    assertThat(passCount)
        .isCloseTo((long) (TestObjects.LOTS_OF_SPANS.length * sampleRate), withPercentage(3));
  }
  ...
  public void idempotent() {
    ...
    assertThat(Stream.of(TestObjects.LOTS_OF_SPANS).parallel().filter(sampler1::isSampled).toArray())
        .containsExactly(Stream.of(TestObjects.LOTS_OF_SPANS).parallel().filter(sampler2::isSampled).toArray());
  }
  ...
  public void zeroMeansDropAllTraces() {
    ...
    assertThat(Stream.of(TestObjects.LOTS_OF_SPANS).parallel().filter(sampler::isSampled).findAny())
        .isEmpty();
  }
  ...
  public void oneMeansKeepAllTraces() {
    ...
    assertThat(Stream.of(TestObjects.LOTS_OF_SPANS).parallel().filter(sampler::isSampled).count())
        .isEqualTo(TestObjects.LOTS_OF_SPANS.length);
  }
