...
public class KafkaSpanCollectorTest {
  ...
  KafkaSpanCollector.Config config = KafkaSpanCollector.Config.builder("localhost:" + kafka.kafkaBrokerPort()).flushInterval(0).build();
  ...
  public void dropsWhenQueueIsFull() throws Exception {
    ...
    assertThat(Codec.JSON.readSpans(readMessages().get(0))).hasSize(1000);
  ...
  public void sendsSpans() throws Exception {
    ...
    assertThat(Codec.JSON.readSpans(messages.get(0))).containsExactly(
        zipkinSpan(1L, "foo"),
        zipkinSpan(2L, "bar")
    );
  }
  ...
  public void submitMultipleSpansInParallel() throws Exception {
    ...
    assertThat(Codec.JSON.readSpans(messages.get(0))).hasSize(400);
  }

  @Test
  public void submitsSpansToCorrectTopic() throws Exception {
    KafkaSpanCollector.Config config = KafkaSpanCollector.Config.builder("localhost:" + kafka.kafkaBrokerPort()).topic("customzipkintopic").build();
    KafkaSpanCollector collector = new KafkaSpanCollector(config, metrics);
